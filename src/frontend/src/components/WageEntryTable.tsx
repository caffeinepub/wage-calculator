...
export default function WageEntryTable({
  entries,
  isLoading,
  onEdit,
}: WageEntryTableProps) {
    ...
    return (
      <div className="space-y-2">
        {["sk1", "sk2", "sk3", "sk4"].map((sk) => (
          <Skeleton key={sk} className="h-10 w-full rounded" />
        ))}
      </div>
    );
  }
              ...
              <TableRow>
                ...
                >
                  <div className="flex flex-col items-center gap-2">
                    ...
                    <p className="text-xs">
                      Klicken Sie auf „Neue Lohnart" um zu beginnen
                    </p>
                  </div>
                </TableCell>
              </TableRow>
              ...
              entries.map((entry, idx) => (
                ...
                >
                  ...
                  <TableCell className="font-mono text-sm text-right border-r border-border py-2.5">
                    {entry.percentageSurcharge != null &&
                    entry.percentageSurcharge > 0 ? (
                      <span className="inline-flex items-center gap-1">
                        <CheckCircle2 className="w-3.5 h-3.5 text-emerald" />
                        <span className="text-emerald font-semibold">
                          {formatNumber(entry.percentageSurcharge)}%
                        </span>
                      </span>
                    ) : (
                      <span className="text-muted-foreground">—</span>
                      ...
                      <AlertDialog>
                        ...
                        <AlertDialogContent>
                          <AlertDialogHeader>
                            ...
                            <AlertDialogDescription>
                              Möchten Sie die Lohnart{" "}
                              <strong>
                                {entry.wageTypeCode} – {entry.description}
                              </strong>{" "}
          ...
          {entries.length > 0 && (
            <TableFooter>
              <TableRow className="bg-table-footer hover:bg-table-footer border-t-2 border-border">
                <TableCell
                  colSpan={8}
                  className="text-sm font-semibold text-foreground py-3 border-r border-border"
                >
