...
import { calculateBetrag, useAddOrUpdateWageEntry } from "../hooks/useQueries";
...
export default function WageEntryForm({
  editingEntry,
  onClose,
  existingCodes,
}: WageEntryFormProps) {
  ...
  useEffect(() => {
    if (editingEntry) {
      const isFixed =
        editingEntry.quantity == null && editingEntry.rate == null;
      setForm({
        wageTypeCode: editingEntry.wageTypeCode,
        description: editingEntry.description,
        quantity:
          editingEntry.quantity != null ? String(editingEntry.quantity) : "",
        rate: editingEntry.rate != null ? String(editingEntry.rate) : "",
        percentageSurcharge:
          editingEntry.percentageSurcharge != null
            ? String(editingEntry.percentageSurcharge)
            : "",
  ...
  const qty = Number.parseFloat(form.quantity) || null;
  const rate = Number.parseFloat(form.rate) || null;
  const surcharge = Number.parseFloat(form.percentageSurcharge) || null;
  const calculatedBetrag = form.isFixedAmount
    ? Number.parseFloat(form.manualAmount) || 0
    : calculateBetrag(qty, rate, surcharge);

  const validate = () => {
    const newErrors: Record<string, string> = {};
    if (!form.wageTypeCode.trim())
      newErrors.wageTypeCode = "Lohnart ist erforderlich";
    if (!form.description.trim())
      newErrors.description = "Bezeichnung ist erforderlich";
    ...
    if (!form.isFixedAmount) {
      if (form.quantity && Number.isNaN(Number.parseFloat(form.quantity)))
        newErrors.quantity = "Ungültige Zahl";
      if (form.rate && Number.isNaN(Number.parseFloat(form.rate)))
        newErrors.rate = "Ungültige Zahl";
      if (
        form.percentageSurcharge &&
        Number.isNaN(Number.parseFloat(form.percentageSurcharge))
      )
        newErrors.percentageSurcharge = "Ungültige Zahl";
    } else {
      if (
        !form.manualAmount ||
        Number.isNaN(Number.parseFloat(form.manualAmount))
      )
        newErrors.manualAmount = "Betrag ist erforderlich";
    }
    setErrors(newErrors);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await mutation.mutateAsync({
        ...
        amount: form.isFixedAmount
          ? Number.parseFloat(form.manualAmount) || null
          : null,
      });
      toast.success(
        editingEntry ? "Lohnart aktualisiert" : "Lohnart hinzugefügt",
      );
      onClose();
    } catch (_err) {
          ...
          {!form.isFixedAmount ? (
            <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-6 gap-4 mt-4">
              ...
              <div className="space-y-1.5">
                ...
                <Select
                  value={form.socialInsuranceFlag}
                  onValueChange={(v) => set("socialInsuranceFlag", v)}
                >
                  <SelectTrigger className="text-sm">
                    <SelectValue />
                  </SelectTrigger>
              ...
              <div className="space-y-1.5">
                ...
                <Select
                  value={form.taxableBenefitFlag}
                  onValueChange={(v) => set("taxableBenefitFlag", v)}
                >
                  <SelectTrigger className="text-sm">
                    <SelectValue />
                  </SelectTrigger>
            ...
            <div className="mt-4 flex items-center gap-3 p-3 bg-emerald-light rounded-md border border-emerald-border">
              ...
              <div className="text-sm">
                ...
                <span className="font-mono font-semibold text-emerald">
                  {calculatedBetrag.toLocaleString("de-DE", {
